...
void a3animation_init_animation(a3_DemoState const* demoState, a3_DemoMode1_Animation* demoMode)
{
	...
	// Blend Nodes and Tree
	{
		...
		demoMode->blendNodePool->nodes[0].data[1] = demoMode->ctrlHS_Walk->animPose;

		demoMode->blendNodePool->nodes[1].data[0] = demoMode->ctrlHS_Idle->animPose;
		demoMode->blendNodePool->nodes[1].data[1] = demoMode->ctrlHS_Walk->animPose;
